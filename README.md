...
This repository **is** the Unity project — clone it and open the folder directly in
Unity Hub.

## ▶ Play in 30 seconds

1. Open this repo folder as a project in **Unity Hub** (Unity **2022.3 LTS+**,
   Built-in or URP — both work). If Hub asks, pick any 2022.3 editor.
2. **Edit ▸ Project Settings ▸ Player ▸ Active Input Handling** → set to **Both**
   (or *Input Manager (Old)*). The scripts use the legacy `Input` API.
3. From the menu bar, click **ViceBay ▸ Create ViceBay_MainScene**. This generates
   `Assets/Scenes/ViceBay_MainScene.unity` with a single `GameBootstrap` object and
   adds it to Build Settings.
4. Press **Play**. The entire Vice Bay map builds itself.
...
A Unity game must be built from the Editor — you need Unity 2022.3 with the matching
**Build Support** module installed in Unity Hub (Windows / macOS / Linux).

1. Install the target's Build Support in Unity Hub (e.g. *Windows Build Support*).
2. Menu bar → **ViceBay ▸ Create ViceBay_MainScene** (once).
3. Menu bar → **ViceBay ▸ Build ▸ Windows (.exe)** (or macOS / Linux).
   Output goes to `Builds/Windows/ViceBayEmpire.exe`.
...
```
Build logic lives in `Assets/Editor/BuildScript.cs`. You can only produce a Windows
`.exe` on a machine whose Unity install has Windows Build Support.
